...
public class MuteCommand extends Command {
    ...
    public void onCalled(CommandEvent event) {
        ...
        if (event.getArgs().size() > 0) {
            ...
            commandManager.getBrianBot().getGuild().addRoleToMember(member, commandManager.getBrianBot().getRole("Muted")).queue();
            event.reply("Muted " + user.getAsMention() + " for " + reason);
        } else {
